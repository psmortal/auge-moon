-- 建库
CREATE DATABASE IF NOT EXISTS aglaia DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_bin;
...
CREATE TABLE `aglaia`.`user_profile` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  ...
  `card_type` TINYINT COMMENT '1 身份证  2 护照 ',
  `card_number` varchar(20),
  `address` varchar(100),
  `user_source` varchar(10) COMMENT '参考字典 user_source',
  `mobile` varchar(11) NOT NULL,
  `email` varchar(50),
  `sex` int DEFAULT 3 COMMENT '性别（1男 2女 3未知）',
  `wxappid` varchar(50),
  `user_type` tinyint(4) NOT NULL COMMENT '用户类型 参考字典 user_type',
  `status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '0 禁用 1 在用 ',
  `valid` tinyint(4) NOT NULL DEFAULT '1' COMMENT '0 无效 1 有效',
...
CREATE TABLE base_picture_group
(
  ...
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='图片组';

CREATE TABLE `aglaia`.`field_profile` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  ...
  `address` varchar(50),
  `size` varchar(20),
  `surround_type` INT NOT NULL COMMENT  '周围环境类型，见字典environment',
  `linkman` VARCHAR(20) ,
  `owner_type` INT NOT NULL COMMENT '身份类型 1 个人  2 公司',
  `description_prohibte` VARCHAR(500) COMMENT '禁止说明',
  `description_management` VARCHAR(500) COMMENT '物业要求说明',
  `start_time_business` TIME NOT NULL COMMENT '营业开始时间',
  `end_time_business` TIME NOT NULL COMMENT '营业结束时间',
  `customer_rate` INT COMMENT '消费者比例特征 见字典 customer_rate',
  `cbd_id` INT COMMENT '商圈 id',
  `filed_type` INT COMMENT '1 场地 2广告',
  `place_type` INT COMMENT '1 室内 2室外',
  `extra_service` INT COMMENT '1 停车位 2水电 3物料过夜 4座椅',
  `card_type` INT COMMENT '1身份证 2营业执照',
  `card_number` VARCHAR(20) COMMENT '证件号码',
  `card_pictures` VARCHAR(200) COMMENT '证件照片',
  ...
  `update_time` datetime NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='场地';

CREATE TABLE `aglaia`.`field_profile_shot` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  ...
  `card_type` INT COMMENT '1身份证 2营业执照',
  `card_number` VARCHAR(20) COMMENT '证件号码',
  `card_picture` VARCHAR(200) COMMENT '证件照片',
  `lng` VARCHAR(20) COMMENT '经度',
  `lat` VARCHAR(20) COMMENT '维度',
  `valid` tinyint(4) NOT NULL DEFAULT '1' COMMENT '0 无效 1 有效',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='场地租赁快照';

CREATE TABLE `aglaia`.`field_rent` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `filed_id` bigint(20) NOT NULL COMMENT '场地id',
  `filed_shot_id` bigint(20) NOT NULL COMMENT '场地快照id',
  `rent_amount` DECIMAL(10,2) not null,
  `rent_amount_discount` DECIMAL(10,2) not null,
  `rent_unit` INT NOT NULL COMMENT '1日 2月 3年',
  `need_reserve` TINYINT NOT NULL DEFAULT 0 COMMENT '是否需要预约',
  `rent_time` INT NOT NULL COMMENT '租期',
  `start_time` DATETIME NOT NULL ,
  `end_time` DATETIME NOT NULL ,
  `holiday_able` TINYINT COMMENT '节假日是否可用 0 否 1 是',
  `installment_able` TINYINT COMMENT '支持分期 0 否 1 是',
  `status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '1发布中 2待支付 3成交 4到期 8退款 9作废',
  `valid` tinyint(4) NOT NULL DEFAULT '1' COMMENT '0 无效 1 有效',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='场地租赁信息';
...
CREATE TABLE `activity_sponsor` (
  ...
  `tags` varchar(100) DEFAULT NULL comment '标签，关键字',
  `verified` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否认真 0否 1是',
  `valid` tinyint(4) NOT NULL DEFAULT '1' COMMENT '0 无效 1 有效',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='活动主办方';
...
CREATE TABLE `activity_profile` (
  ...
  `start_time` datetime NOT NULL,
  `end_time` datetime NOT NULL,
  `picture_group_id` bigint(20) DEFAULT NULL,
  `address` varchar(500) NOT NULL,
  `price` decimal(10,2) NOT NULL,
  `tags` varchar(100) DEFAULT NULL comment '标签，关键字',
  `spec_order` varchar(10) DEFAULT NULL comment '特殊排序',
